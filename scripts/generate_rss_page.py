    ...
    <div class="col-lg-12">
        <div class="alert alert-info">
            ...
            <div class="row text-center">
                ...
                <div class="col-md-3">
                    <strong>{(now + timedelta(hours=1)).strftime('%H:%M')} (London)</strong><br>
...
"""
        ...
        # Update archive section in main news page
        self.update_archive_section(archive_date, articles, summaries)

    def extract_key_topics(self, articles, summaries):
        """Extract key topics from articles and summaries for archive description"""
        topics = set()

        # Look at "Don't Miss" content
        recent_articles = []
        recent_articles.extend(articles.get('this_week', []))
        recent_articles.extend(articles.get('this_month', [])[:10])

        # Extract topics from article titles
        for article in recent_articles[:5]:  # Top 5 articles
            title_lower = article.get('title', '').lower()
            ...
            if 'whatsapp' in title_lower:
                topics.add('WhatsApp')
            if 'side conversation' in title_lower:
                topics.add('side conversations')
            if 'brand' in title_lower and 'membership' in title_lower:
                topics.add('brand management')
            if 'jira' in title_lower:
                topics.add('Jira integration')
            if 'sunshine' in title_lower:
                topics.add('Sunshine API')
            if 'voice' in title_lower and 'triage' in title_lower:
                topics.add('voice triage')
            if 'attachment' in title_lower and 'expiration' in title_lower:
                topics.add('attachment controls')
            if 'agent' in title_lower and ('timeout' in title_lower or 'profile' in title_lower):
                topics.add('agent management')
            if 'custom object' in title_lower and 'api' in title_lower:
                topics.add('export APIs')
            if 'gmail' in title_lower or 'exchange' in title_lower:
                topics.add('email connectors')

        # Look at release notes summary
        release_summary = summaries.get('release_notes', {}).get('summary', '')
        if 'copilot' in release_summary.lower():
            topics.add('Copilot updates')
        if 'voice' in release_summary.lower():
            topics.add('voice features')

        return list(topics)[:3]  # Max 3 topics

    def update_archive_section(self, archive_date, articles, summaries):
        """Update the archive section in news.md with new entry"""
        ...
        # Extract key topics for this archive
        topics = self.extract_key_topics(articles, summaries)
        topic_str = ' & '.join(topics) if topics else 'platform updates'

        # Format date for display
        date_obj = datetime.strptime(archive_date, '%Y-%m-%d')
        display_date = date_obj.strftime('%d %B').lstrip('0')  # e.g., "22 September"

        # Create new archive entry
        new_entry = f'                        <li><a href="/news-{archive_date}/" class="text-dark">{display_date} - {topic_str}</a></li>'

        # Find the archive section and insert new entry at the top of September list
        lines = content.split('\n')
        archive_section_found = False
        september_section_found = False

        for i, line in enumerate(lines):
            if '<h6 class="text-primary">September 2025</h6>' in line:
                september_section_found = True
                continue

            if september_section_found and '<ul class="list-unstyled archive-links">' in line:
                # Insert new entry right after the opening <ul> tag
